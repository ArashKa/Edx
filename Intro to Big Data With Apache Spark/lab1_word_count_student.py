...
# TODO: Replace <FILL IN> with appropriate code
pluralRDD = wordsRDD.map(lambda s: makePlural(s))
...
# TODO: Replace <FILL IN> with appropriate code
a= wordsRDD.map(lambda x: (x,1))
b=a.reduceByKey(lambda x, y:x+y)
wordCountsCollected=b.collect()                                 
...
def wordCount(wordListRDD):
    a=wordListRDD.map(lambda x:(x,1))
    b=a.reduceByKey(lambda x, y:x+y)
...
# TODO: Replace <FILL IN> with appropriate code
finalRDD= wordCount(shakeWordsRDD)
top15WordsAndCounts=finalRDD.takeOrdered(15,key=lambda x:-x[1])
