...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    Ave=sum(IDandRatingsTuple[1])/float(len(IDandRatingsTuple[1]))
    return IDandRatingsTuple[0],(len(IDandRatingsTuple[1]),Ave)
...
def computeError(predictedRDD, actualRDD):
    ...
    totalError = squaredErrorsRDD.values().reduce(lambda a,b:a+b)
...
validationForPredictRDD = validationRDD.map(lambda (u_Id, m_Id, rating):(u_Id, m_Id))
...
testForPredictingRDD = testRDD.map(lambda (u_Id, m_Id, rating):(u_Id, m_Id))
...
testForAvgRDD = testRDD.map(lambda (u_Id,m_Id, rating):(u_Id,m_Id,trainingAvgRating))
...
#                      .<FILL IN>)
                      .map(lambda (m_Id, m_Title): (0, m_Id))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (m_Id,(num, avg)):(m_Id, num))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (u_Id,m_Id, rating):(m_Id, rating))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .map(lambda (m_Id,(rating, num)):(rating, m_Id, num))
                       .filter(lambda (rating, m_Id, num):num>75)
                       )
