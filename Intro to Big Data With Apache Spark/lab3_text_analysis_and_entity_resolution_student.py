...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (a,b):tf(b).keys())
...
def dotprod(a, b):
    ...
    return sum(a[i]*b[i] for i in a.keys() if i in b.keys())
...
#print trueDupsRDD.values().take(1)
avgSimDups = trueDupsRDD.values().reduce(lambda a,b:a+b)/float(trueDupsCount)

nonDupsRDD = (sims
             .subtractByKey(trueDupsRDD))
avgSimNon = nonDupsRDD.values().reduce(lambda a,b:a+b)/float(nonDupsRDD.count())
