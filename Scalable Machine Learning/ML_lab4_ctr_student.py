...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    indices=[OHEDict[feature] for feature in rawFeats]
    sorted_indices=sorted(indices)
    values=np.ones(len(indices))
    return SparseVector(numOHEFeats, sorted_indices, values)
...
sampleDistinctFeats = (sampleDataRDD
                       .flatMap(lambda sample : sample)
                       .distinct()
                       )
print type(sampleDistinctFeats)
...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    # Here I make an RDD of our input data 
    
    # let's make a distionary of our RDD.
    dict_features=inputData.flatMap(lambda x:x).distinct()
                  
                   
    hot_dict=dict_features.zipWithIndex().collectAsMap()
    return hot_dict
...
url = 'labs.criteo.com/wp-content/uploads/2015/04/dac_sample.tar.gz'
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    for item in rawFeats:
        if item in OHEDict:
            indices.append(OHEDict[item])
    sorted_indices=sorted(indices)
    return SparseVector(numOHEFeats, sorted_indices, np.ones(len(indices)))
...
def evaluateResults(model, data):
    ...
    return data.map(lambda x: computeLogLoss(model.predict(x.features), x.label)).sum()/data.count()
...
logLossValLR0 = OHEValidationData.map(lambda x: computeLogLoss(model0.predict(x.features), x.label)).sum()/OHEValidationData.count()
...
def parseHashPoint(point, numBuckets):
    ...
    data=point.split(',')
    parsed_point=parsePoint(point)
    features=hashFunction(numBuckets,parsed_point,printMapping=False)
    return LabeledPoint(data[0],SparseVector(numBuckets,features))
...
def computeSparsity(data, d, n):
    ...
    """
    return float(data.map(lambda x: len(x.features.values)).sum())/(d*n)
