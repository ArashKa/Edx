...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda lp:LabeledPoint(lp.label-1922.0,lp.features))
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda point:(point.label,averageTrainYear))
...
def twoWayInteractions(lp):
    ...
    """
    product=list(itertools.product(lp.features,lp.features))
    product=map(lambda x:x[0]*x[1],product)
    return LabeledPoint(lp.label, list(lp.features)+product)
